...
export function UsersSection(): ReactElement {
  ...
  const [filter, setFilter] = useState<'all' | 'tool' | 'members'>('all')
  ...
  const adminMut = useMutation({
    mutationFn: (args: { userId: string; is_tool_admin: boolean }) =>
      putAdminUserAdminStatus(args.userId, { is_tool_admin: args.is_tool_admin }),
  ...
  const counts = useMemo(() => {
    ...
    return {
      all: list.length,
      tool: list.filter((u) => u.is_tool_admin).length,
      members: list.filter((u) => !u.is_tool_admin).length,
    }
  }, [dirQ.data])
  ...
  const rows = useMemo(() => {
    ...
    return list.filter((u) => {
      if (filter === 'tool' && !u.is_tool_admin) return false
      if (filter === 'members' && u.is_tool_admin) return false
                    ...
                    <div className="flex flex-wrap justify-end gap-1.5">
                      {u.is_tool_admin ? (
                        <Btn
                          ...
                          title={
                            selfRow
                              ? 'Another tool admin must revoke your access.'
                              : undefined
                          }
                          onClick={() =>
                            adminMut.mutate({ userId: u.user_id, is_tool_admin: false })
                          }
                        >
                          Remove tool admin
                        </Btn>
                        ...
                        <Btn
                          ...
                          onClick={() =>
                            adminMut.mutate({ userId: u.user_id, is_tool_admin: true })
                          }
                        >
                          Grant tool admin
                        </Btn>
