...
import {
  ...
  getAdminStudioMemberBudgets,
  patchAdminStudioBudget,
  patchAdminStudioMemberBudget,
...
export function BudgetsSection(): ReactElement {
  ...
  const memberBudgetsQ = useQuery({
    ...
    queryFn: () => getAdminStudioMemberBudgets(selectedStudioId as string),
    enabled: scope === 'builder' && selectedStudioId != null,
  })

  const patchStudioBudget = useMutation({
    mutationFn: (vars: {
      studioId: string
      capUsd?: number
      ...
      return patchAdminStudioBudget(vars.studioId, body)
    },
    onSuccess: async () => {
  ...
  const patchMemberBudget = useMutation({
    mutationFn: ({
      ...
      patchAdminStudioMemberBudget(studioId, userId, {
