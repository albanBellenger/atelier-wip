/**
 * Platform routing buckets: maps token_usage.call_type → one of four DB use_case keys.
 * Must stay aligned with backend ``use_case_for_call_type`` in llm_policy_service.py.
 */
import { llmCallTypeLabel } from './llmCallTypeLabels'
...
/** call_type strings that resolve to each routing bucket (best-effort inventory). */
...
/** Human-readable list of agents (call types) in this bucket. */
export function routingBucketAgentsSummary(useCase: string): string {
  ...
  return types.map((ct) => llmCallTypeLabel(ct)).join(' · ')
}
