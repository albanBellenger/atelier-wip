...
**API:** `GET/PATCH /admin/embeddings/reindex-policy` (tool admin). Runtime embedding calls use encrypted keys on [`LlmProviderRegistry`](../backend/app/models/llm_policy.py) plus the **embeddings** routing rule; the **embedding model catalog** (`embedding_model_registry`) remains optional metadata (cost, docs)—not the sole source for which model runs in production.
